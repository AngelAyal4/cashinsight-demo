...
- Stack: Next.js 15 (App Router) + React 19 + TypeScript strict + TailwindCSS + Tremor + MongoDB (Mongoose) + Zod
...
3. **Seguridad** (SECURITY-CHECKLIST):
   ...
   - Input validado server-side (Zod en API routes), headers de seguridad, SQL/ORM parametrizado (revisar código de rutas)
   - Rate limiting en endpoints públicos (login/register/forgot/reset), JWT con httpOnly
4. **Calidad**: lint + build + tsc. Si el proyecto tiene gates (cobertura mínima), verificarlos.
